...
function initSprites(img) {
 	...
 	medal  = new  Sprite(img, 174, 216.5, 22, 22);

	s_bird = [
		new Sprite(img, 152, 152, 22, 24),
		new Sprite(img, 152, 152, 22, 24),
		new Sprite(img, 152, 152, 22, 24)
	];

	s_bg = new Sprite(img, 330, 0, 300, 310);
	s_bg.image = "wbg.png"; // save background color
	...
	s_text = {
		...
		GetReady:   new Sprite(img, 59, 255, 87, 21)
	}
	s_buttons = {
	...
	s_splash = new Sprite(img,   0, 114,  59, 61);

	s_numberS = new Sprite(img, 0, 176, 6,  8);
	s_numberB = new Sprite(img, 0, 187.5, 7, 11);
